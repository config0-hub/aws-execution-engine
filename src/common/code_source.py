...
def fetch_ssm_values(paths: List[str], region: Optional[str] = None) -> Dict[str, str]:
    ...
    Each SSM parameter value is a base64-encoded JSON dict of env var key-value pairs.
    All decoded dicts are merged into a single result.
    """
    if not paths:
        return {}
...
def group_git_orders(
    ...
    """Group orders by code source type.

    Returns:
        git_groups: dict mapping (repo, commit_hash) -> list of (order_index, order)
        s3_indices: list of order indices that use S3 source

    Orders with no s3_location and no git_repo are skipped (commands-only).
    They will be handled separately by the caller (e.g. Phase 3 in repackage).
    """
    ...
    for i, order in enumerate(orders):
        if getattr(order, "s3_location", None):
            ...
            if not repo:
                # Commands-only order — no code source, skip grouping
                continue
