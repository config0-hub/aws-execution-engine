...
@retry_on_throttle
def reserve_order_for_dispatch(
    run_id: str,
    order_num: str,
    dynamodb_resource=None,
) -> bool:
    """Atomically flip an order from `queued` -> `dispatching`.

    Returns True if the reservation succeeded (caller owns the dispatch),
    False if another orchestrator already reserved the order (caller must skip).
    This is the first half of the two-step dispatch pattern that prevents
    duplicate worker invocations across concurrent orchestrator runs.
    """
    table = _get_table("AWS_EXE_SYS_ORDERS_TABLE", dynamodb_resource)
    now = int(time.time())
    try:
        table.update_item(
            ...
            UpdateExpression="SET #status = :dispatching, last_update = :last_update",
...
def put_event(
    ...
    ttl_seconds = int(os.environ.get("AWS_EXE_SYS_EVENT_TTL_SECONDS", 86400 * 90))
    item = {
        "trace_id": trace_id,
        "sk": sk,
...
def acquire_lock(
    ...
    try:
        table.put_item(
            ...
            # Steal the lock if:
            #   1. no lock row exists,
            #   2. the previous holder marked it completed, OR
            #   3. the stored ttl is in the past (holder crashed; DynamoDB's
            #      background TTL cleanup can lag by up to 48h, so we enforce
            #      expiry at the application level here).
            ConditionExpression=(
                Attr("pk").not_exists()
                | Attr("status").eq("completed")
                | Attr("ttl").lt(Decimal(now))
            ),
        )
        return True
    except table.meta.client.exceptions.ConditionalCheckFailedException:
