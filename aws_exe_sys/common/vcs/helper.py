...
Providers are looked up from the package-level registry in
``aws_exe_sys.common.vcs.registry``. To add a new provider (bitbucket, gitlab,
gitea…), define a ``VcsProvider`` subclass and call
``register_provider(MyProvider())`` at import time.
"""
...
from .registry import UnknownVcsProviderError, get_provider, list_providers
...
class VcsHelper:
    ...
    def __init__(self, provider: str = "github"):
        try:
            self._provider: VcsProvider = get_provider(provider)
        except UnknownVcsProviderError:
            supported = ", ".join(sorted(list_providers()))
            raise ValueError(
